...
test("Slack onboarding launches directly and verifies connection on return", async () => {
  ...
  try {
    ...
    releaseStatus();
    await settle();
    assert.match(element.textContent ?? "", /添加到 Slack/);
    assert.doesNotMatch(element.textContent ?? "", /已将 QM 添加到 Slack/);
    ...
    await settle();
    assert.match(element.textContent ?? "", /已将 QM 添加到 Slack/);
    ...
    window.dispatchEvent(new window.Event("focus"));
    await settle();
    assert.match(element.textContent ?? "", /无法检查 Slack 安装状态/);
    window.open = () => null;
    element.querySelector<HTMLButtonElement>(".welcome-slack")!.click();
    await settle();
    assert.match(element.textContent ?? "", /请允许打开新标签页/);
    window.open = () => popup as unknown as Window;
    launchCode = 502;
    element.querySelector<HTMLButtonElement>(".welcome-slack")!.click();
    await settle();
    assert.equal(closed, true);
    assert.match(element.textContent ?? "", /无法开始安装 Slack/);
