...
export function readConfig(env: NodeJS.ProcessEnv): IdLoginConfig {
  return {
    issuer: (env.IDLOGIN_ISSUER ?? `http://localhost:${env.PORT ?? 8193}`).replace(/\/$/, ""),
...
export function createIdLoginHandler(deps: IdLoginDeps): (req: IncomingMessage, res: ServerResponse) => Promise<void> {
  ...
  async function token(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (!idOk || !secretOk) {
      res.writeHead(
        401,
        noStore({ "content-type": "application/json", "www-authenticate": `Basic realm="qm-idlogin"` }),
      );
...
export async function bootIdLogin(
  cfg: IdLoginConfig,
  databaseUrl: string | undefined,
): Promise<{ handle: (req: IncomingMessage, res: ServerResponse) => Promise<void>; kid: string }> {
  const users = databaseUrl ? await createPostgresUserRegistry(databaseUrl) : createMemoryUserRegistry();
  ...
  return { handle, kid: signingKey.kid };
}
