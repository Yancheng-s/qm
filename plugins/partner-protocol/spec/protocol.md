...
| `POST /v1/assemble`                                                       | **否** | 任何 `502` 都先 `GET /v1/employees` 核对，确认没建出来再重试；`201` 之后的局部失败（`grantFailures`/`soul`/`standingOrders`）没有单独补写端点，只能重建员工 |
...
- 单次 `assemble` 最多授权 **50** 条技能。这是刻意的闸门：授权越多，后端每轮要装载的技能上下文越大。请只授权当前员工真正需要的技能。
